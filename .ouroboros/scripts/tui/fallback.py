...
class FallbackScreenBuffer:
    ...
    def __init__(self, width: int, height: int, output_stream=None, start_row: int = 1):
        """
        ...
        for row in rows_to_render:
            ...
            for col in range(self.width):
                self._previous[row][col] = Cell(
                    self._current[row][col].char, self._current[row][col].style
                )
...
class FallbackWindow:
    ...
    def write_line(self, y: int, text: str, attr: str = "", pad: bool = True) -> None:
        """Write a full line."""
