...
from tests.pbt_framework import property_test, MultilineGenerator, StringGenerator, Generator
import random
...
class MultilineContentGenerator(Generator[str]):
    ...
    def generate(self, rng: random.Random) -> str:
        ...
        for _ in range(num_lines):
            ...
            line = ''.join(rng.choices('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!?;:()-_=+[]{}', k=line_len))
            lines.append(line)
        return '\n'.join(lines)
    ...
    def generate(self, rng: random.Random) -> str:
        ...
        for _ in range(num_lines):
            ...
            line = pattern.format(
                ...
                value=''.join(rng.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=rng.randint(1, 10))),
                ...
                comment=''.join(rng.choices('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ', k=rng.randint(5, 30))),
            )
            lines.append(line)
        return '\n'.join(lines)
...
class LongSingleLineGenerator(Generator[str]):
    ...
    def generate(self, rng: random.Random) -> str:
        ...
        return ''.join(rng.choices('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!?', k=length))
...
from utils.badge import create_paste_marker, expand_markers, is_paste_marker, parse_paste_marker, extract_paste_content
    ...
    def test_paste_marker_round_trip_code_content(self, content: str):
        ...
        **Validates: Requirements 7.1, 7.5-7.6, 16.3**
        ...
        self.assertEqual(
            content,
            expanded,
            f"Round-trip failed for code content"
        )
    ...
    def test_parse_paste_marker_line_count(self, content: str):
        ...
        **Validates: Requirements 7.1-7.2, 7.5**
        ...
        self.assertEqual(
            content,
            parsed_content,
            f"Parsed content differs from original"
        )
        ...
        **Validates: Requirements 7.5-7.6, 16.3**
        ...
        for content in test_contents:
            ...
            self.assertEqual(
                via_extract,
                via_expand,
                f"Extraction methods differ for content"
            )
            self.assertEqual(
                content,
                via_extract,
                f"Extracted content differs from original"
            )
        ...
        **Validates: Requirements 7.5-7.6, 16.3**
        ...
        for content in test_cases:
            ...
            self.assertEqual(
                content,
                expanded,
                f"Newline pattern not preserved for: {repr(content)}"
            )
